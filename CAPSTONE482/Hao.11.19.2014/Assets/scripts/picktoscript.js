...
function Update () {
	...
	if(Input.GetMouseButtonDown(0) && !canpick && pickObj.GetComponent(pickedupobj).refusethrow !=true && holdingBall){    	canpick = true;
...
function OnGUI(){
	GUI.Label (Rect (Screen.width/2, Screen.height/2.1, Screen.width/1, Screen.height/2), "X");
	
	if(guipick && canpick){
		GUI.Label (Rect (Screen.width/2, Screen.height/2, Screen.width/1, Screen.height/2), "Pick Up");
	}
}